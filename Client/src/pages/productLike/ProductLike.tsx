...
const ProductLike: FunctionComponent = () => {
    ...
    const [cartLike, setLike] = useState<ILikeList[]>([]);
    ...
    const handleDeleteLike = (id: number) => {
        ...
        if (confirmDelete) {
            setLike(cartLike.filter((item) => item.id !== id));
            deleteLike(id).then(() => {
        ...
        <div className="w-full relative bg-color-white overflow-hidden flex flex-col items-start justify-start pt-0 px-0 pb-[50px] box-border leading-[normal] tracking-[normal]">
            ...
            <section className="self-stretch flex flex-col items-start justify-start pt-0 px-0 pb-[22px] box-border max-w-full">
                <div className="self-stretch flex flex-col items-center justify-start pt-[61px] pb-[97px] pr-[25px] pl-5 relative gap-[1px] text-left text-29xl text-black font-poppins">
                    ...
                    <div className="w-[115px] flex flex-row items-start justify-end py-0 pr-0.5 pl-[7px] box-border">
                        <div className="h-[133px] flex-1 relative">
                            <h1 className="m-0 absolute top-[61px] left-[0px] text-inherit font-medium font-inherit inline-block min-w-[106px] z-[1] mq1050:text-19xl mq450:text-10xl">Like</h1>
                    ...
                    <div className="flex flex-row items-start justify-start gap-[6px] text-base">
                        ...
                        <div className="relative font-light inline-block min-w-[35px] z-[1]">Like</div>
                    </div>
                </div>
                ...
                <div className="self-stretch bg-color-white flex flex-col items-start justify-start pt-[72px] px-[50px] pb-[63px] box-border gap-[30px] max-w-full text-left text-base text-black font-poppins lg:flex-wrap mq750:gap-[15px] mq750:pt-[47px] mq750:px-[50px] mq750:pb-[41px] mq750:box-border mq450:pl-5 mq450:pr-5 mq450:box-border">
                    ...
                    {cartLike.length > 0 ? (
                        cartLike.map((like) => (
                            <div key={like.id} className="self-stretch flex flex-row items-center justify-between py-4 px-4 gap-4">
                                <Link to={`/product/${like.product?.id}`} style={{ textDecoration: 'none',color:"black" }}>
                                    <div className="flex-1 flex items-center gap-2">
                                        <img className="h-16 w-16 object-cover cursor-pointer" alt="" src={like.product?.image || '/default-image.png'} />
                                        <span>{like.product?.name || 'Unknown Product'}</span>
                                    </div>
                                </Link>
                                <div className="flex items-center ml-auto"> {/* Sử dụng class ml-auto của Tailwind CSS để đẩy sang phải */}
                                    <span className="text-left">{like.product?.price || 'N/A'}₫</span>
                                </div>
                                <div className="w-[100px] text-left">
                                    <IconButton size="small" onClick={() => handleDeleteLike(like.id)}>
                                        <DeleteIcon />
                                    </IconButton>
                                </div>
                            </div>
